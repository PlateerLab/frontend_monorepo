...
import type { AgentflowDetail } from '@xgen/types';

// ─────────────────────────────────────────────────────────────
// API Response Types
// ─────────────────────────────────────────────────────────────

interface AgentflowDetailAPIResponse {
  id: number;
...
function transformAgentflowDetail(response: AgentflowDetailAPIResponse): AgentflowDetail {
  let status: AgentflowDetail['status'] = 'active';
  ...
  return {
    ...
    sharePermissions: response.share_permissions as AgentflowDetail['sharePermissions'],
...
// ─────────────────────────────────────────────────────────────
// Agentflow Storage API
// ─────────────────────────────────────────────────────────────

export async function listAgentflowsDetail(): Promise<AgentflowDetail[]> {
  const api = createApiClient();
  const response = await api.get<{ workflows: AgentflowDetailAPIResponse[] }>('/api/workflow/list/detail');
  return (response.data.workflows || []).map(transformAgentflowDetail);
}

export async function loadAgentflow(workflowId: string, userId?: number): Promise<unknown> {
  const api = createApiClient();
...
export async function duplicateAgentflow(workflowId: string, userId?: number): Promise<{ success: boolean }> {
  const api = createApiClient();
  const query = userId ? `?user_id=${userId}` : '';
...
export async function deleteAgentflow(workflowId: string): Promise<{ success: boolean }> {
  const api = createApiClient();
