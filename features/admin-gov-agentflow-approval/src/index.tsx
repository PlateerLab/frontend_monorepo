...
function deriveDetailStatus(d: AgentflowDetail): GovernanceStatus {
...
/*  Component                                                          */
/* ------------------------------------------------------------------ */
const AdminGovAgentflowApprovalPage: React.FC<RouteComponentProps> = () => {
  ...
  const [detail, setDetail] = useState<AgentflowDetail | null>(null);
  ...
  const openDetail = useCallback(async (wf: ApprovalRequest) => {
    ...
    try {
      const res = await getAgentflowApprovalDetail(wf.workflow_id);
      setDetail(res);
    } catch {
      setDetail(null);
    } finally {
  ...
  const submitReview = useCallback(async () => {
    ...
    try {
      await reviewGovernanceAgentflow(reviewTarget.workflow_id, reviewAccept, reviewComment);
      setReviewOpen(false);
      setDetailOpen(false);
      await loadData();
    } catch {
  ...
  return (
    <ContentArea
      title={t('admin.governance.agentflowApproval.title')}
      description={t('admin.governance.agentflowApproval.description')}
    >
      {/* Summary cards — clickable as filters */}
        <div className="grid grid-cols-4 gap-4">
          {([
            ...
            { key: 'pending' as const, variant: 'warning' as const, label: t('admin.governance.agentflowApproval.pending'), value: summary.pending },
            { key: 'approved' as const, variant: 'success' as const, label: t('admin.governance.agentflowApproval.approved'), value: summary.approved },
            { key: 'rejected' as const, variant: 'error' as const, label: t('admin.governance.agentflowApproval.rejected'), value: summary.rejected },
          ] as const).map((c) => (
            <StatCard
              key={c.key}
              label={c.label}
        ...
        <div className="flex items-center gap-3">
          <div className="w-72">
            <SearchInput
              value={search}
              onChange={setSearch}
              placeholder={t('admin.governance.agentflowApproval.searchPlaceholder')}
            />
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/30 text-left">
                  ...
                  >
                    {t('admin.governance.agentflowApproval.governanceStatus')} {renderSortIcon('governanceStatus')}
                  </th>
                  <th className="px-4 py-3 font-semibold text-xs text-muted-foreground tracking-wide">
                    {t('admin.governance.agentflowApproval.reviewer')}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold text-xs text-muted-foreground tracking-wide cursor-pointer select-none"
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <StatusBadge variant={cfg.badgeStatus}>
                            {t(`admin.governance.agentflowApproval.${govStatus}`)}
                          </StatusBadge>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          ...
                          >
                            <Button size="sm" variant="outline" onClick={() => openDetail(r)}>
                              {t('admin.governance.agentflowApproval.viewDetail')}
                            </Button>
                            {govStatus === 'pending' && (
                              <>
                                <Button size="sm" onClick={() => openReview(r, true)}>
                                  {t('admin.governance.agentflowApproval.approve')}
                                </Button>
                                <Button
                                  size="sm"
                                  variant="outline"
                                ...
                                >
                                  {t('admin.governance.agentflowApproval.reject')}
                                </Button>
                              </>
                            )}
                          </div>
        ...
        {detailOpen && (
          <Modal
            isOpen
            onClose={closeDetail}
            title={t('admin.governance.agentflowApproval.detailTitle')}
          >
            <div className="flex flex-col gap-4 p-4" style={{ maxWidth: 920 }}>
              {detailLoading ? (
                <div className="flex items-center justify-center py-10 text-muted-foreground">
                  {t('admin.governance.agentflowApproval.loading')}
                </div>
              ) : detail ? (
                <>
                  {/* Basic info grid */}
                  <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                    {([
                      ...
                      { label: t('admin.governance.agentflowApproval.version'), value: `v${detail.current_version ?? 1}` },
                      {
                        label: t('admin.governance.agentflowApproval.nodeEdgeInfo'),
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs text-muted-foreground">
                        {t('admin.governance.agentflowApproval.governanceStatus')}
                      </span>
                        ...
                        {(() => {
                          ...
                          return (
                            <StatusBadge variant={STATUS_CONFIG[s].badgeStatus}>
                              {t(`admin.governance.agentflowApproval.${s}`)}
                            </StatusBadge>
                          );
                        })()}
                      </span>
                  ...
                  {detail.governance_review_comment && (
                    <div className="mt-2">
                      <h4 className="text-sm font-medium text-foreground mb-1">
                        {t('admin.governance.agentflowApproval.reviewComment')}
                      </h4>
                  ...
                  {/* Canvas placeholder */}
                  {/* TODO: AgentflowStoreMiniCanvas is not yet available in monorepo. Add canvas view here when ported. */}
                    ...
                    <div className="mt-2 bg-muted/30 rounded-xl p-4 border border-border">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="text-sm font-medium text-foreground">
                          {t('admin.governance.agentflowApproval.nodeDetail')} — {selectedNode.nodeName}
                        </h4>
                        <Button
                          variant="ghost"
                          size="icon"
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-3">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="text-xs text-muted-foreground">
                            {t('admin.governance.agentflowApproval.category')}
                          </span>
                      ...
                      {Object.keys(selectedNode.parameters ?? {}).length > 0 && (
                        <div className="mt-3">
                          <span className="text-xs text-muted-foreground block mb-2">
                            {t('admin.governance.agentflowApproval.parameters')} ({Object.keys(selectedNode.parameters).length})
                          </span>
                      ...
                      {(selectedNode.inputs?.length ?? 0) > 0 && (
                        <div className="mt-3">
                          <span className="text-xs text-muted-foreground block mb-2">
                            {t('admin.governance.agentflowApproval.inputs')} ({selectedNode.inputs.length})
                          </span>
                          <div className="flex flex-col gap-1.5">
                  ...
                  {(detail.node_summaries?.length ?? 0) > 0 && (
                    <div className="mt-2">
                      <h4 className="text-sm font-medium text-foreground mb-2">
                        {t('admin.governance.agentflowApproval.nodeSummary')}
                      </h4>
                          ...
                          <thead>
                            <tr className="bg-muted/30 text-left">
                              <th className="px-3 py-2 font-semibold text-xs text-muted-foreground tracking-wide">
                                {t('admin.governance.agentflowApproval.nodeName')}
                              </th>
                              ...
                              <th className="px-3 py-2 font-semibold text-xs text-muted-foreground tracking-wide">
                                {t('admin.governance.agentflowApproval.paramCount')}
                              </th>
                              <th className="px-3 py-2 font-semibold text-xs text-muted-foreground tracking-wide">
                                {t('admin.governance.agentflowApproval.ioInfo')}
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-border">
                ...
                <div className="flex items-center justify-center py-10 text-muted-foreground">
                  {t('admin.governance.agentflowApproval.loadFailed')}
                </div>
              )}
            </div>
          </Modal>
        ...
        {reviewOpen && reviewTarget && (
          <Modal
            ...
            title={
              reviewAccept
                ? t('admin.governance.agentflowApproval.approveTitle')
                : t('admin.governance.agentflowApproval.rejectTitle')
            }
          >
            <div className="flex flex-col gap-4 p-4" style={{ maxWidth: 480 }}>
              ...
              <div>
                <label className="text-xs text-muted-foreground block mb-2">
                  {t('admin.governance.agentflowApproval.commentLabel')}
                </label>
                <textarea
                  value={reviewComment}
                  ...
                  placeholder={
                    reviewAccept
                      ? t('admin.governance.agentflowApproval.approveCommentPlaceholder')
                      : t('admin.governance.agentflowApproval.rejectCommentPlaceholder')
                  }
                  rows={4}
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-border">
                ...
                >
                  {reviewSubmitting
                    ? t('admin.governance.agentflowApproval.submitting')
                    : reviewAccept
                      ? t('admin.governance.agentflowApproval.approve')
                      : t('admin.governance.agentflowApproval.reject')
                  }
...
const feature: AdminFeatureModule = {
  id: 'admin-gov-agentflow-approval',
  name: 'AdminGovAgentflowApprovalPage',
  adminSection: 'admin-governance',
  routes: {
    'admin-gov-agentflow-approval': AdminGovAgentflowApprovalPage,
  },
};
